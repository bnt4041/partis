...
**Notas y figuras.** El panel **Notas** (♪) tiene la paleta de figuras (de cuadrada a semifusa) y de silencios: se arrastran directamente al pentagrama y sueltan ahí (la altura la decide dónde se suelten), o se pulsan una vez para **armarlas** - el botón queda resaltado - y entonces cada clic sobre el pentagrama coloca una nota en ese punto, sin tener que volver a coger el botón cada vez; un segundo clic sobre el mismo botón, elegir otra figura o `Esc` lo desarma. Junto a ellas: alteraciones (incluidos doble sostenido y doble bemol), **puntillo y doble puntillo**, y **grupos especiales** (dosillo, tresillo, cuatrillo, cincillo, seisillo, septillo y nonillo). El puntillo y el grupo especial armado se aplican a lo que coloques a continuación; sobre notas ya seleccionadas, el botón **Aplicar a selección** (junto al selector de grupo especial) o el menú contextual → *Grupo especial* las convierte en ese grupo, y **Quitar grupo** lo deshace. Colocar una nota nueva justo en medio de un grupo especial ya completo (por ejemplo, entre la 2ª y 3ª nota de un tresillo) no lo parte por dentro: la nota nueva se coloca justo después del grupo entero en su lugar, para no robarle una posición a una de sus notas.

**Suprimir borra de verdad.** Seleccionar una nota (o silencio) y pulsar **Suprimir** la quita por completo - no deja un silencio de la misma duración en su lugar - así que el compás se queda más corto hasta que se rellene con otra cosa.

**Acordes (notas dobles).** Tres formas:
- Haz clic justo encima o debajo de una nota ya escrita: se le añade esa altura como nota de acorde (misma duración que la nota, sin necesidad de tener nada armado ni marcar ninguna casilla); si la alteración del panel Notas tiene algo elegido, se aplica a la nota añadida igual que a una nota nueva. Un clic sobre una altura que el acorde ya tiene no la duplica.
- Después, cada nota del acorde se selecciona y se trata como una nota independiente (salvo la figura/duración, que es una sola para todo el acorde): un clic normal sobre la cabeza de nota que quieras - aunque estén muy juntas, como un acorde de 2ª, donde abcjs las dibuja una al lado de la otra en vez de apiladas - la selecciona a ella sola (se resalta solo esa cabeza, no todo el acorde) y **Suprimir** la borra sin tocar las demás. Clic derecho justo sobre ella → *Altura* → alterar, subir/bajar un tono o una octava (solo esa nota), o *Acorde* → *Borrar esta nota*. Si el acorde se queda con una única nota, deja de ser acorde.
...
**Cambiar de armadura a mitad de pieza.** Clic derecho sobre el pentagrama (en un hueco, no sobre una nota) → *Añadir armadura* inserta un cambio de tonalidad en ese punto - y reescribe automáticamente todas las notas que le siguen (en ese pentagrama, hasta el próximo cambio de armadura) para que sigan sonando exactamente igual que antes: si una nota escrita como `F` sonaba F♯ con la armadura anterior, al cambiar a una sin sostenidos pasa a escribirse `^F` para seguir sonando F♯ - la armadura nueva cambia cómo se escribe de ahí en adelante, nunca lo que suena. Las notas que ya tuvieran su propia alteración explícita no se tocan (esas suenan como están escritas, tenga la armadura los sostenidos/bemoles que tenga).
...
El icono **Archivo** del lateral (también accesible desde el menú **Menú ▾**, ver abajo) abre la lista de partituras guardadas por cualquiera de la organización (no son privadas por usuario), con quién la guardó y cuándo; "Abrir" carga esa partitura en el editor (sustituyendo la actual) y "🗑" la borra.

**El menú general (Menú ▾)** de la barra superior reúne, en un único desplegable siempre visible (no hace falta tener una partitura abierta), todo lo que no se toca cada pocos segundos como sí se hace con la paleta de notas:

- **Nuevo** abre el panel de "Nueva composición" (con IA o en blanco) - lo mismo que el icono **Crear** del lateral.
- **Archivo** abre el mismo panel que el icono del lateral (la lista de partituras guardadas).
- Con una partitura abierta, además aparecen:
  - **Guardar** pregunta el título y sobrescribe la partitura (o crea una nueva la primera vez). Cada vez que sobrescribe una ya guardada, primero avisa con un diálogo de confirmación, y el contenido anterior no se pierde: el backend lo archiva automáticamente antes de sobrescribir (si el contenido no cambió, no archiva nada).
  - **Nueva versión** es la vía contraria: archiva lo que haya *ahora mismo* en el editor como una versión propia, sin tocar la partitura ya guardada - útil para dejar un punto de control de un estado intermedio sin comprometerlo como la copia "oficial".
  - **Versiones** lista esas copias archivadas (por cualquiera de las dos vías) con fecha y quién la guardó; abrir una la carga en el editor - igual que abrir una partitura del archivo, sustituyendo el trabajo actual - sin sobrescribir nada por sí sola: hace falta pulsar Guardar para conservarla (lo que a su vez archiva lo que hubiera antes, así que nunca se pierde nada por recuperar una versión antigua).
  - **Autoguardado** (se recuerda por navegador, como el metrónomo) hace que, 15 segundos después de la última edición, se repita el mismo guardado que haría "Guardar" - mismo título (el `T:` del ABC), mismo `score_id` - pero en silencio: sin preguntar el título ni interrumpir con el diálogo de confirmación. Solo actúa si la partitura ya se guardó una vez a mano y si de verdad hay cambios desde el último guardado (propio o automático); el propio botón del menú se resalta mientras está activado.
  - **Descargar MusicXML** / **Descargar MIDI** exportan la partitura tal cual está en el editor (no hace falta haberla guardado).
- **Perfil** muestra usuario, rol y organización (una consulta rápida a `GET /api/auth/org/me`, que cualquier rol con organización puede llamar sobre sí mismo).
- **Cerrar sesión** - lo único que antes vivía suelto en la barra superior (icono ⏻ junto al nombre de usuario) y ahora está aquí también.

**Deshacer / rehacer** se quedan fuera de ese menú, como botones ↶/↷ propios junto a él (o `Ctrl+Z` / `Ctrl+Y`, también `Ctrl+Shift+Z`): recorren el historial de cambios de la partitura abierta - cualquier edición cuenta: notas, acordes, letra, cabecera, editar el ABC a mano... Escribir directamente en el panel **ABC** agrupa las pulsaciones seguidas en un solo paso de deshacer en vez de una por tecla. El historial se reinicia al empezar una pieza nueva, abrir otra partitura guardada o cargar una versión anterior - deshacer nunca cruza de una pieza a otra.
